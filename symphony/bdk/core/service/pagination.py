...
async def offset_based_pagination(func: Callable[[int, int], Awaitable[T]],
                                  chunk_size=50, maxNumber=None) -> AsyncGenerator[T, None]:
    ...
    :param maxNumber: the maximum total number of items to retrieve. If not specified or set to None, it will fetch all
    ...
    """
    if maxNumber is not None and maxNumber <= 0:
        return
    ...
    while chunk:
        for item in chunk:
            ...
            item_count += 1
            if maxNumber is not None and item_count == maxNumber:
